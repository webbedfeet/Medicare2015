...
```{r Suppl-Figure-4, fig.height=9.5, fig.width=7.5}
...
depression <- import(path(drop_dir, 'raw/PROJ4_COMORB_TKA_WHITE.csv')) %>% 
  select(hrr, depress) %>% mutate(depress = depress*100)
comorb_tka <- comorb_tka %>% select(hrr, prop, smr3, condition) %>% 
  spread(condition, prop) %>% 
  left_join(depression) %>% 
  gather(condition, prop, -hrr, -smr3)
comorb_tka %>% filter(condition %in% c('depress','chf','diabetes')) %>% 
...
#### Figure 6. Mean annual number of primary total knee arthroplasties performed per surgeon among white Medicare beneficiaries, by Health Referral Region. Regions are ordered from low to high based on their observed/expected ratio.

```{r Suppl-Figure-6, cache = TRUE}
library(ggridges)
